...
import (
	"context"
	"fmt"
	"github.com/ava-labs/avalanche-network-runner/examples/local/timestampvm/config"
...
func run(log logging.Logger, binaryPath string) error {
	...
	timestampVMNetworkConfig := config.BuildConfig(amountOfNodes)
	for i := 0; i < amountOfNodes; i++ {
		timestampVMNetworkConfig.NodeConfigs[i].ImplSpecificConfig = utils.NewLocalNodeConfigJsonRaw(binaryPath)
	}
	// Create the network
	nw, err := local.NewNetwork(log, timestampVMNetworkConfig, "")
	if err != nil {
		return err
	}
